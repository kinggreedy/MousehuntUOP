...
function MHT_iframeLoaded() {
	var UOP_iframe = document.getElementById("UOP_iframe");
	UOP_iframe.height = Math.max(
		Math.max(UOP_iframe.contentDocument.body.scrollHeight, UOP_iframe.contentDocument.documentElement.scrollHeight),
		Math.max(UOP_iframe.contentDocument.body.offsetHeight, UOP_iframe.contentDocument.documentElement.offsetHeight),
		Math.max(UOP_iframe.contentDocument.body.clientHeight, UOP_iframe.contentDocument.documentElement.clientHeight));
	UOP_iframe.width = 646; //set to max
}

//THESE ARE the script worker
//TOOLBOX AREA
//When use Toolbox, use this: onclick='UOP_global.UOP_callbackFunctions.UOP_toolboxShowContent(" + i + "," + j + ");return true;'
...
function UOP_sound()
{
	...
	if (UOP_global.UOP_objects.UOP_hornheader.className.indexOf('hornsounding') != -1)
	{
		UOP_global.UOP_callbackFunctions.UOP_soundhornsounding();
		setTimeout(function () { (UOP_global.UOP_callbackFunctions.UOP_sound)() }, 100);
		return;
	}
	else if (UOP_global.UOP_objects.UOP_hornheader.className.indexOf('hornsounded') != -1)
	{
		UOP_global.UOP_callbackFunctions.UOP_soundhornsounded();
		setTimeout(function () { (UOP_global.UOP_callbackFunctions.UOP_sound)() }, 100);
		return;
	}

	UOP_global.UOP_callbackFunctions.UOP_syncUser(true,UOP_global.UOP_callbackFunctions.UOP_soundhornwaiting);
}
function UOP_soundhornsounding()
{
	UOP_global.UOP_callbackFunctions.UOP_autosounding();
}
function UOP_soundhornsounded()
{
	UOP_global.UOP_callbackFunctions.UOP_autosounded();
}

function UOP_soundhornwaiting()
{
...
function UOP_toolboxShowContent(url)
{
	var popup = new jsDialog();
	popup.addToken('{*content*}', '<iframe id="UOP_iframe"></iframe>');
	popup.show();
	var iframeRequest = document.getElementById('UOP_iframe');
...
function UOP_removeAds()
{
	...
	if (rightCol != null)
	{
		while (rightCol.childElementCount > 1)
			if (rightCol != null) UOP_removeObject(rightCol.lastChild,rightCol);//rightCol.style.display = "none";
	}
	else
	{
		var rightCol = document.getElementById('rightCol'); //facebook ads
		if (rightCol == null) return; //no ads
		UOP_removeObject(rightCol,null);
		return;
	}
	if (UOP_global.settings.UOP_ads != 2) return;
}
...
function UOP_SimulateTabBar(x)
{
	var UOP_main_div = document.getElementById('UOP_tabbarContents_page');
	var UOP_main_tab = document.getElementById('UOP_tabbarControls_page').getElementsByTagName('li');
	
	for (i = 0;i < UOP_main_div.childElementCount;++i) UOP_main_div.childNodes[i].className = "inactive";
	for (i = 0;i < UOP_main_tab.length;++i) UOP_main_tab[i].className = "inactive";
	UOP_main_div.childNodes[x].className = "active";
	UOP_main_tab[x].className = "active";
}
...
//ADD FEATURES AREA
function UOP_asyncUser()
{
	if ('undefined' == typeof user)
	{
		setTimeout (function() {UOP_asyncUser();},100);
	}
	else
	{
		UOP_global.UOP_callbackFunctions.UOP_syncUser(false,null);
		UOP_global.UOP_callbackFunctions.UOP_page_process();
	}
}

function UOP_syncUser(forceSync,callbackFunction) //forceSync
{
	if (forceSync == false)
	{
		UOP_global.UOP_user.user = user;
	}
	else
	{
		var XMLHTTP_next_activeturn_seconds;
		var UOP_XMLHttpRequest = new XMLHttpRequest();
		UOP_XMLHttpRequest.open("GET", "managers/ajax/abtest.php", true);
		UOP_XMLHttpRequest.setRequestHeader("Cache-Control","no-cache, must-revalidate");
		UOP_XMLHttpRequest.setRequestHeader("Pragma","no-cache");
		UOP_XMLHttpRequest.setRequestHeader("If-Modified-Since","Sat, 1 Jan 2000 00:00:00 GMT");
		UOP_XMLHttpRequest.onreadystatechange = function()
		{
			if (UOP_XMLHttpRequest.readyState === 4)
			{
				if (UOP_XMLHttpRequest.status == 200)
				{
					UOP_global.UOP_user = eval('(' + UOP_XMLHttpRequest.responseText + ')');
					user = UOP_global.UOP_user.user;
		...
		UOP_XMLHttpRequest.send(null);
	}
}

function UOP_syncTime()
{
	UOP_global.UOP_callbackFunctions.UOP_syncUser(false,null);
...
/* Schedule
UOP_puzzleStandardReaction => pause schedule
